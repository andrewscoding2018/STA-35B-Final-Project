...
```{r}
joined_frame <- left_join(children, payments, by = "CASE_NUM")
...
children_and_payments2 <- joined_frame %>% mutate(
  age = 2017 - (year(DOB_fixed) + (month(DOB_fixed) / 12) + (day(DOB_fixed) / 365.25))) %>% 
  group_by(AP_ID) %>% 
  summarize(num_child = length(unique(ID)), 
    num_pymnt = length(PYMNT_AMT), 
    mean_age = mean(age))
```

```{r}
# Omit any NA values and check number of rows removed
children_and_payments_cln2 <- na.omit(children_and_payments2)
nrow(children_and_payments2) - nrow(children_and_payments_cln2)

# Preview 
head(children_and_payments_cln2)
```
...
```{r}
cor(children_and_payments_cln2$mean_age, children_and_payments_cln2$num_pymnt)
```
