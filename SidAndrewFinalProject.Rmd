...
```{r}
number <- children %>% select(CASE_NUM, ID) %>% group_by(CASE_NUM) %>% mutate(numberchildren = length(ID)) %>% ungroup(CASE_NUM) %>% summarize(mean(numberchildren)) # Mean number of children

children %>% select(CASE_NUM, ID)%>% group_by(CASE_NUM) %>% mutate(numberchildren = length(ID)) %>%
  ggplot(mapping = aes(x = numberchildren)) + geom_histogram(binwidth = 1) + geom_vline(xintercept = as.numeric(number), size = 5, color = "red") # Graph
```
...
This is the same child because we grouped by ID, which identifies the child. This child has 12 cases of child support, as a child needing support. 
...
Not every parent from the parents data file is present in the payments data. This likely means that there are parents who did not pay child support
...
* The two peaks in our data could be due to different payment plans, where some parents choose to pay a larger amount more infrequently and others choose to pay a smaller amount more frequently

* We can try a linear regression test at a 95% confidence level to see if total number of payments and total amount paid are related

  + Step 1: check assumptions and state null hypothesis:
    - First, let's see if the association is roughly linear

```{r}
f <- payments %>% filter(DATE >= as.Date("2015-05-01")) %>% group_by(AP_ID, DATE) %>% summarize(num_pymnts = length(PYMNT_AMT), total_pymnts = sum(PYMNT_AMT))
f %>% ggplot(mapping = aes(x = num_pymnts, y = total_pymnts)) + geom_point()
```

    - H0: there is no relationship between number of payments and total amount in payments
    - Ha: there *is* a relationship between number of payments and total amount in payments

  + Step 2/3: find the t-statistic and p-value
```{r}
summary(lm(total_pymnts ~ num_pymnts, data = f))
```
  ...
  + Step 5: state a conclusion
    - Since the p-value is less than 0.05, we reject the null hypothesis that there is no relationship between the number of payments and total amount in payments
...
```
Overall, people don't tend to pay extremely high or low amounts; rather, they sit somewhere in the middle
...
* We want to test whether "parents deemed responsible for more children are more likely to make either a larger number of payments or a larger total payment amount over this period." For this, we can compare the correlations of "number of children vs number of payments" and "number of children vs total payments" with a correlation test
  + Step 1: check assumptions and state null and alternate hypothesis
    - We assume that the variables are both quantitative and have a linear relationship
```{r}
children_and_pymnts %>% ggplot(mapping = aes(x = num_child, y = num_pymnt)) + geom_point()
```

```{r}
children_and_pymnts %>% ggplot(mapping = aes(x = num_child, y = total_pymnt)) + geom_point()
```
    - Since both graphs are approximately normally distributed, we can proceed
    - H0: there is no correlation between the variables
    - Ha: there is a correlation between the variables
  + Step 2: find t-statistic / p-value
```{r}
cor.test(children_and_pymnts$num_child, children_and_pymnts$num_pymnt)
cor.test(children_and_pymnts$num_child, children_and_pymnts$total_pymnt)
```
  + Step 4: make a decision
    - Since both p-values are less than 0.05, we reject both null hypothesis
  + Step 5: state a claim
    - Since both p-values are less than 0.05, we reject the null hypotheses that there is no correlation between the variables
* Furthermore, looking at the individual correlations, we can see from the respective Pearson Correlation Coefficients that parents deemed responsible for more children are more likely to make a larger total payment over the time period rather than make more  payments (if only by a little bit)
...
children_and_pymnts2 <- joined_frame %>% mutate(
...
```{r}
children_and_pymnts2["total_pymnt"][is.na(children_and_pymnts2["total_pymnt"])] <- 0

# Omit any NA values
children_and_pymnts2 <- na.omit(children_and_pymnts2)

# Preview 
head(children_and_pymnts2)
```

* We now look at the correlation between mean age and the number of payments made using a correlation test at the 95% confidence level

  + Step 1: check assumptions and state null and alternate hypothesis
    - First, let's graph the variables to see if they have an approximately linear relationship
    
```{r}
plot(children_and_pymnts2$mean_age, children_and_pymnts2$total_pymnt)
```
    
    - H0: there is no correlation between mean age and total payment
    - Ha: there *is* a correlation between mean age and total payment
    
  + Step 2/3: find the t-statistic and p-value

```{r}
cor.test(children_and_pymnts2$mean_age, children_and_pymnts2$total_pymnt)
```
  ...
  + Step 5: state a conclusion
    - Since the p-value is less than 0.05, we reject the null hypothesis that there is no correlation between mean age and total payment. However, the Pearson Correlation Coefficient appears to be very small; therefore, there isn't a strong correlation between the mean age of the children and the number of payments made
...
temp_df %>% ggplot(mapping = aes(x = as.factor(AP_ADDR_ZIP), y = log(total_pymnt))) + geom_boxplot()
```
...
```
    ...
    - Assumption 3: we assume that the population variances are equal across responses for each category
```{r}
left_join(children_and_pymnts, parents, by = "AP_ID") %>% select(AP_ID, num_child, total_pymnt, AP_ADDR_ZIP) %>% group_by(AP_ADDR_ZIP) %>% summarize(sd = sd(total_pymnt))
```


  + Step 2/3: find F-statistic and p-value
```{r}
parents$factor_ADDR <- as.factor(parents$AP_ADDR_ZIP)
model <- aov(children_and_pymnts$total_pymnt ~ parents$factor_ADDR)
summary(model)
```
  + Step 4: make a decision
...
* We can answer this question using multiple linear regression We set up the following hypothesis test with a confidence level of 95%:
  + Step 1: make assumptions and state null / alternate hypothesis
    - First, we'll create the graphs and see if we can continue
```{r}
linear_model <- lm(total_pymnt ~ num_child + mean_age, data = children_and_payments2)
plot(linear_model)
```

    - We conclude assumptions have been met, so we can continue
    - H0: The number of children and mean ages of the children are not predictors of the total payment amount (Bi = ... = Bk = 0)
    - Ha: The number of children and mean ages of the children *are* predictors of the total payment amount (at least one Bj != 0)
...
```
  + Step 4: make a decision
    - Since the p-value is less than 0.05, we reject the null hypothesis
  + Step 5: state a conclusion
    - Since the p-value is less than 0.05, we reject the null hypothesis that the number of children and mean ages of the children are not predictors of the total payment amount
...
There is a weak correlation between n, the total amount of payments. 
...
It looks like Payment Amount, Payment SRC and Payment Type have a statistically significant relation to the likelihood that the parent will have a cv less than 0.5. 

```{r}


payment_Summary <- payments %>% na.omit() %>% group_by(DATE, AP_ID) %>% summarize(totalDailySum = sum(PYMNT_AMT))


summarySD <- payment_Summary %>% group_by(DATE) %>% filter(n() > 1) %>% summarize(sd = sd(totalDailySum, na.rm = TRUE), mean = mean(totalDailySum, na.rm = TRUE))
summarySD %>% mutate(cv = (sd/mean))

summarySD2 <- summarySD %>% mutate(cv = sd/mean) %>% mutate(level = cv < 0.5)
summarySD2

cvAttributes <- left_join(payments, summarySD2, by = "DATE") %>% mutate(cv = sd/mean)
cvAttributes

summary(aov(level ~ CASE_NUM + PYMNT_AMT + DATE + PYMNT_SRC + PYMNT_TYPE + AP_ID, data = cvAttributes))
