...
* There are two peaks in our data: one around 2.5k payments and one around 6k payments. The reason we have a bi-modal distribution in our data is because the higher range of payments are typically being made by males

```{r}
# Graph for male APs
left_join(parents, payments, by = "AP_ID") %>% filter(SEX_CD == "M") %>% group_by(DATE) %>% summarize(num_pymt = length(PYMNT_AMT)) %>% filter(DATE >= as.Date("2015-05-01")) %>% ggplot(mapping = aes(x = num_pymt)) + 
  geom_density()

# Graph for female APs
left_join(parents, payments, by = "AP_ID") %>% filter(SEX_CD == "F") %>% group_by(DATE) %>% summarize(num_pymt = length(PYMNT_AMT)) %>% filter(DATE >= as.Date("2015-05-01")) %>% ggplot(mapping = aes(x = num_pymt)) + 
  geom_density()
```

Let's now look at the number of male vs female absent parents:

```{r}
sum(parents$SEX_CD == "M")
sum(parents$SEX_CD == "F")
```

Since there are more males in the data set, the distribution of number of payments paid more closely resembles that of male absent parents
...
```{r}
cor(children_and_pymnts2$mean_age, children_and_pymnts2$total_pymnt)
```
...
cases[-which(cases$LAST_PYMNT_DT == ""),]

pyments <-full_join(payments, parents, by = "AP_ID") %>% group_by(AP_ID, COLLECTION_DT) %>% mutate(sum = sum(na.omit(PYMNT_AMT)))

pyments1sd <- pyments %>% group_by(AP_ID, COLLECTION_DT) %>% mutate(sd1 = sd(sum), mean1 = mean(na.omit(sum)))

pyments1sd %>% summarize(cor(sd1, mean1))
...
paymentgraph <- pyments1sd %>% mutate(cv = sd1/mean1)
paymentgraph %>% ggplot(mapping = aes(x = cv)) +
  geom_histogram()
